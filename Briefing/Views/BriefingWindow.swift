...
struct BriefingWindowView: View {
        ...
        VStack(alignment: .leading, spacing: 0) {
            HStack {
                Label("Tasks", systemImage: "checkmark.circle")
                    .font(.system(size: 15, weight: .semibold))
                Spacer()
                if let cachedAt = tasksCachedAt {
                    Text("cached \(DateFormatting.time.string(from: cachedAt)) · read-only")
                        .font(.system(size: 12))
    ...
    private func loadAll() async {
        ...
        // Fill briefing slots from the shared iCloud cache (generated by the
        // scheduler, the popover, or the other Mac)
    ...
    private func loadTasks() async {
        let tasks = await thingsService.fetchAllTasksWithCache(
            cacheDirectoryPath: settings.taskDirectoryPath
        )
        let cachedAt = await thingsService.lastThingsCacheDate
        let running = await thingsService.isThingsRunning()

        allTasks = tasks
        tasksCachedAt = cachedAt
        // Writes need live Things on THIS machine — cached tasks mean there is
        // nothing to write to (and no local auth token on the work Mac)
        writesEnabled = running && cachedAt == nil
